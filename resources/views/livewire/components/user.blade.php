  ...
  <x-jet-dropdown align="right" width="48">
    ...
    <x-slot name="content">
      @auth
        ...
        <!-- Authentication -->
        <x-jet-dropdown-link class="hover:cursor-pointer" onClick="logoutClick()">
          Logout
        </x-jet-dropdown-link>
        <form method="POST" action="{{ route('logout') }}" x-data hidden>
          @csrf
  ...
  const logoutClick = () => {
    Swal.fire({
      ...
      Swal.fire({
        ...
        timer: 1500
      }).then(() => document.getElementById("logout-link").click());
    });
  }
</script>
